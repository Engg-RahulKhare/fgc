...
class _RouteOptionsCardState extends State<LaunchManagementScreen> {
      ...
      for (String line in csvLines) {
        if (line.isNotEmpty) {
          ...
          print(row);
           ...
           List <num> processrow = processRow(row);
          
          processedRows.add(processrow);
        }
      }
      // Convert the processed rows to binary forma t
      print(processedRows);
  ...
  List<num> processRow(List<num> row) {
  List<num> reversedData = List<num>.from(row);
  ...
  for (int i = 0; i < row.length; i++) {
    ...
    switch (byteSize) {
      case 2:
        reversedData[i] = reverse2Byte([row[i].toInt()], 1)[0];
        break;
      case 4:
        reversedData[i] = reverse4Byte([row[i].toInt()], 1)[0];
        break;
      case 8:
        if (row[i] is double) {
          reversedData[i] = reverse8ByteDouble([row[i].toDouble()], 1)[0];
        } else {
          reversedData[i] = reverse8Byte([row[i].toInt()], 1)[0];
        }
        break;
...
List<int> reverse2Byte(List<int> data, int number) {
  int b1, b2;
...
List<int> reverse4Byte(List<int> data, int number) {
  int w1, w2;
  for (int i = 0; i < number; ++i) {
    w1 = data[i] >> 16;
...
List<int> reverse8Byte(List<int> data, int number) {
  int w1, w2;
  for (int i = 0; i < number; ++i) {
    w1 = data[i] >> 32;
...
List<double> reverse8ByteDouble(List<double> data, int number) {
  int d1, d2;
  for (int i = 0; i < number; ++i) {
...
List<int> convertToBinary(List<num> row) {
  ...
  for (num value in row) {
    if (value is int) {
      // Convert integers to 8-byte binary representation in little-endian
      List<int> bytes = List.generate(8, (index) => (value >> (index * 8)) & 0xFF);
      binaryData.addAll(bytes.reversed);
    } else if (value is double) {
      // Convert doubles to 8-byte binary representation in little-endian
      ByteData byteData = ByteData(8);
      byteData.setFloat64(0, value, Endian.little);
      binaryData.addAll(byteData.buffer.asUint8List());
    }
  }
...
final String serverUrli = "http://134.32.20.102:5005";
  void launcharinc() async {
    try {
      await http.post(Uri.parse('$serverUrli/launcharinc'));
  ...
  void stoparinc() async {
    try {
      await http.post(Uri.parse('$serverUrli/stoparinc'));
